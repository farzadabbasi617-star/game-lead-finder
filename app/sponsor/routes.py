...
def layout(title: str, body: str, token: str = '') -> HTMLResponse:
    ...
    return HTMLResponse(f'<!doctype html><html lang="fa"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{h(title)}</title>{css}</head><body><div class="wrap"><div class="hero"><h1>{h(title)}</h1><div class="muted">پیدا کردن کانال‌های گیمینگ مناسب تبلیغات اسپانسری، با آمار و امتیازدهی</div><a class="btn btn2" href="/">بانک لیدها</a> <a class="btn btn2" href="/analytics">📊 آنالیتیکس</a> <a class="btn btn2" href="/sponsor">🎯 کانال‌های اسپانسری</a> <a class="btn btn2" href="/sponsor/discover">🔍 کشف کانال</a></div>{body}</div>{js}</body></html>')


def _stats(db: Session) -> dict:
    total = db.scalar(select(func.count(SponsorChannel.id))) or 0
    telegram = db.scalar(select(func.count(SponsorChannel.id)).where(SponsorChannel.platform == 'telegram')) or 0
    high_score = db.scalar(select(func.count(SponsorChannel.id)).where(SponsorChannel.ad_score >= 50)) or 0
    booked = db.scalar(select(func.count(SponsorChannel.id)).where(SponsorChannel.status == 'booked')) or 0
    return {'total': total, 'telegram': telegram, 'high_score': high_score, 'booked': booked}


def _build_export_query(db: Session, status: str, category: str, q: str):
    ...
    if q:
        like = f'%{q}%'
        stmt = stmt.where(or_(SponsorChannel.title.ilike(like), SponsorChannel.username.ilike(like)))
    return stmt.order_by(desc(SponsorChannel.ad_score))
...
def sponsor_index(db: Session = Depends(get_db), token: str = Query(''), status: str = Query(''), category: str = Query(''), q: str = Query(''), sort: str = Query('ad_score'), min_members: int = Query(0, ge=0), min_score: int = Query(0, ge=0, le=100), limit: int = Query(100, ge=1, le=500), msg: str = Query('')):
    stats = _stats(db)
    stmt = select(SponsorChannel)
    if status: stmt = stmt.where(SponsorChannel.status == status)
    if category: stmt = stmt.where(SponsorChannel.category.ilike(f'%{category}%'))
    ...
    if min_members: stmt = stmt.where(SponsorChannel.member_count >= min_members)
    ...
    body = f'''<div class="grid4"><div class="stat-card">کل کانال‌ها<b style="color:#7c3aed">{stats["total"]}</b></div><div class="stat-card">تلگرام<b style="color:#2563eb">{stats["telegram"]}</b></div><div class="stat-card">امتیاز بالا (≥۵۰)<b style="color:#12b76a">{stats["high_score"]}</b></div><div class="stat-card">رزرو شده<b style="color:#f79009">{stats["booked"]}</b></div></div>
    {f'<div class="card hint">{h(msg)}</div>' if msg else ''}
    <div class="card"><h3>🔍 جستجو و فیلتر</h3><form method="get" action="/sponsor"><input name="q" placeholder="جستجو" value="{h(q)}" style="min-width:240px"><select name="status">{status_opts}</select><input name="category" placeholder="دسته" value="{h(category)}" style="width:120px"><label>حداقل عضو <input type="number" name="min_members" value="{min_members}" min="0" style="width:100px"></label><label>حداقل امتیاز <input type="number" name="min_score" value="{min_score}" min="0" max="100" style="width:80px"></label><select name="sort">{sort_opts}</select><button>فیلتر</button></form></div>
    <div class="card"><h3>📋 لیست کانال‌های اسپانسری ({len(channels)} مورد)</h3><div style="overflow-x:auto"><table><thead><tr><th>کانال</th><th style="text-align:center;width:120px">آمار</th><th style="width:200px">امتیازها</th><th style="width:140px">اطلاعات تبلیغ</th><th style="width:140px">عملیات</th></tr></thead><tbody>{rows or '<tr><td colspan="5" style="text-align:center;padding:30px" class="muted">هنوز کانالی کشف نشده.</td></tr>'}</tbody></table></div></div>
    <div class="card"><a class="btn" href="/sponsor/export.xlsx?status={h(status)}&category={h(category)}&q={h(q)}">📥 Excel</a> <a class="btn2 btn" href="/sponsor/export.csv?status={h(status)}&category={h(category)}&q={h(q)}">📥 CSV</a></div>'''
    return layout('🎯 کانال‌های اسپانسری گیمینگ', body, token)
...
def discover_page(db: Session = Depends(get_db)):
    ...
    qcb = ''.join(f'<label style="display:block;margin:4px 0"><input type="checkbox" name="queries" value="{h(q)}" {"checked" if i < 6 else ""}> {h(q)}</label>' for i, q in enumerate(GAMING_QUERIES))
    body = f'''<div class="card"><h3>🔍 کشف کانال‌های جدید</h3><p class="muted">با جستجوی وب، کانال‌های تلگرامی مرتبط با گیمینگ رو پیدا می‌کنه.</p><form method="post" action="/sponsor/discover"><h4>عبارت‌های جستجو:</h4>{qcb}<div style="margin-top:12px"><label>نتیجه هر جستجو <input type="number" name="max_results" value="10" min="1" max="30" style="width:80px"></label><label>حداقل امتیاز <input type="number" name="min_score" value="20" min="0" max="100" style="width:80px"></label></div><div style="margin-top:12px"><button>🚀 شروع کشف</button></div></form><div class="hint"><b>نکته:</b> اگر API Key تنظیم نشده باشه، فقط از لیدهای موجود استخراج می‌کنه.</div></div>
    <div class="card"><h3>➕ افزودن کانال دستی</h3><form method="post" action="/sponsor/manual"><input name="channel_url" placeholder="لینک کانال (مثلاً https://t.me/channel)" required style="min-width:300px"><input name="title" placeholder="نام کانال"><input name="category" placeholder="دسته‌بندی"><button class="btn2">افزودن</button></form></div>'''
    return layout('کشف کانال‌های اسپانسری', body)
...
async def discover_run(db: Session = Depends(get_db), max_results: Annotated[int, Form()] = 10, min_score: Annotated[int, Form()] = 20, queries: Annotated[list[str] | None, Form()] = None):
    result = await discover_sponsor_channels(db, queries=queries, max_results_per_query=max_results, min_ad_score=min_score)
    msg = f"کشف: {result['queries_run']} جستجو، {result['channels_found']} کانال، {result['new_saved']} جدید، {result['duplicates']} تکراری."
...
async def manual_add(db: Session = Depends(get_db), channel_url: Annotated[str, Form()] = '', title: Annotated[str, Form()] = '', category: Annotated[str, Form()] = ''):
    ...
    ch = SponsorChannel(platform='telegram' if 't.me' in channel_url else 'instagram', channel_url=channel_url, username=username, title=title or username or 'نامشخص', description=description, member_count=member_count, avg_views=avg_views, engagement_rate=engagement_rate, category=category.strip() or None, source='manual', status='new')
...
def export_csv(db: Session = Depends(get_db), status: str = '', category: str = '', q: str = ''):
    channels = list(db.scalars(_build_export_query(db, status, category, q)).all())
...
def api_sponsor(db: Session = Depends(get_db), token: str = Query(''), status: str = '', category: str = '', min_score: int = 0, limit: int = Query(50, ge=1, le=500)):
    stmt = select(SponsorChannel)
    if status: stmt = stmt.where(SponsorChannel.status == status)
    if category: stmt = stmt.where(SponsorChannel.category.ilike(f'%{category}%'))
    if min_score: stmt = stmt.where(SponsorChannel.ad_score >= min_score)
