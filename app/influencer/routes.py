...
def layout(title: str, body: str) -> HTMLResponse:
    ...
    return HTMLResponse(f'<!doctype html><html lang="fa"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{h(title)}</title>{css}</head><body><div class="wrap"><div class="hero"><h1>{h(title)}</h1><div class="muted">کشف و مدیریت اینفلوئنسرهای گیمینگ برای همکاری و تبلیغات</div><a class="btn btn2" href="/">بانک لیدها</a> <a class="btn btn2" href="/sponsor">🎯 اسپانسری</a> <a class="btn btn2" href="/influencer">🌟 اینفلوئنسرها</a> <a class="btn btn2" href="/influencer/discover">🔍 کشف اینفلوئنسر</a></div>{body}</div>{js}</body></html>')


def _stats(db: Session) -> dict:
    total = db.scalar(select(func.count(Influencer.id))) or 0
    by_platform = {p: db.scalar(select(func.count(Influencer.id)).where(Influencer.platform == p)) or 0 for p in ['instagram', 'telegram']}
    high_score = db.scalar(select(func.count(Influencer.id)).where(Influencer.collab_score >= 50)) or 0
    collaborating = db.scalar(select(func.count(Influencer.id)).where(Influencer.status == 'collaborating')) or 0
    by_tier = {t: db.scalar(select(func.count(Influencer.id)).where(Influencer.tier == t)) or 0 for t in ['nano', 'micro', 'mid', 'macro', 'mega']}
...
def influencer_index(db: Session = Depends(get_db), platform: str = Query(''), niche: str = Query(''), tier: str = Query(''), q: str = Query(''), sort: str = Query('collab_score'), min_followers: int = Query(0, ge=0), min_score: int = Query(0, ge=0, le=100), status: str = Query(''), limit: int = Query(100, ge=1, le=500), msg: str = Query('')):
    stats = _stats(db)
    stmt = select(Influencer)
    ...
    if min_followers: stmt = stmt.where(Influencer.followers >= min_followers)
    ...
    tier_opts = '<option value="">همه سطوح</option>' + ''.join(f'<option value="{k}" {"selected" if tier==k else ""}>{v[1]}</option>' for k,v in TIER_LABELS.items())
    platform_opts = '<option value="">همه پلتفرم‌ها</option>' + ''.join(f'<option value="{p}" {"selected" if platform==p else ""}>{p}</option>' for p in ['instagram', 'telegram'])
    tier_cards = ''.join(f'<div class="stat-card"><span style="font-size:18px">{TIER_LABELS[t][0]}</span><div class="small">{TIER_LABELS[t][1]}</div><b style="color:{TIER_LABELS[t][2]}">{stats["by_tier"][t]}</b></div>' for t in ['nano','micro','mid','macro','mega'] if stats['by_tier'][t] > 0)

    body = f'''<div class="grid5"><div class="stat-card">کل<b style="color:#be185d">{stats["total"]}</b></div><div class="stat-card">📸 اینستاگرام<b style="color:#e1306c">{stats["by_platform"].get("instagram",0)}</b></div><div class="stat-card">✈️ تلگرام<b style="color:#0088cc">{stats["by_platform"].get("telegram",0)}</b></div><div class="stat-card">⭐ امتیاز بالا<b style="color:#12b76a">{stats["high_score"]}</b></div><div class="stat-card">🤝 همکاری فعال<b style="color:#f79009">{stats["collaborating"]}</b></div></div>
    {f'<div class="grid5">{tier_cards}</div>' if tier_cards else ''}
    {f'<div class="card hint">{h(msg)}</div>' if msg else ''}
    <div class="card"><h3>🔍 جستجو و فیلتر</h3><form method="get" action="/influencer"><input name="q" placeholder="جستجو" value="{h(q)}" style="min-width:240px"><select name="platform">{platform_opts}</select><select name="tier">{tier_opts}</select><select name="status">{status_opts}</select><input name="niche" placeholder="نیچ" value="{h(niche)}" style="width:100px"><label>حداقل فالوور <input type="number" name="min_followers" value="{min_followers}" min="0" style="width:100px"></label><label>حداقل امتیاز <input type="number" name="min_score" value="{min_score}" min="0" max="100" style="width:80px"></label><select name="sort">{sort_opts}</select><button>فیلتر</button></form></div>
    <div class="card"><h3>🌟 لیست اینفلوئنسرها ({len(influencers)} نفر)</h3><div style="overflow-x:auto"><table><thead><tr><th>اینفلوئنسر</th><th style="text-align:center;width:110px">آمار</th><th style="width:180px">امتیازها</th><th style="width:120px">همکاری</th><th style="width:100px">عملیات</th></tr></thead><tbody>{rows or '<tr><td colspan="5" style="text-align:center;padding:30px" class="muted">هنوز اینفلوئنسری کشف نشده.</td></tr>'}</tbody></table></div></div>
    <div class="card"><a class="btn" href="/influencer/export.xlsx">📥 Excel</a> <a class="btn2 btn" href="/influencer/export.csv">📥 CSV</a></div>'''
    return layout('🌟 اینفلوئنسرهای گیمینگ', body)
...
def discover_page():
    ...
    all_q = INSTAGRAM_QUERIES[:8] + TELEGRAM_QUERIES[:6]
    cb = ''.join(f'<label style="display:block;margin:3px 0"><input type="checkbox" name="queries" value="{h(q)}" {"checked" if i < 6 else ""}> {h(q)}</label>' for i, q in enumerate(all_q))
    body = f'''<div class="card"><h3>🔍 کشف اینفلوئنسرهای جدید</h3><p class="muted">با جستجوی وب، اینفلوئنسرهای گیمینگ اینستاگرام و تلگرام رو پیدا می‌کنه.</p><form method="post" action="/influencer/discover"><h4>عبارت‌های جستجو:</h4>{cb}<div style="margin-top:10px"><select name="platform"><option value="both">اینستاگرام + تلگرام</option><option value="instagram">فقط اینستاگرام</option><option value="telegram">فقط تلگرام</option></select><label>نتیجه هر جستجو <input type="number" name="max_results" value="8" min="1" max="20" style="width:80px"></label><label>حداقل امتیاز <input type="number" name="min_score" value="15" min="0" max="100" style="width:80px"></label></div><div style="margin-top:10px"><button>🚀 شروع کشف</button></div></form></div>
...
async def discover_run(db: Session = Depends(get_db), platform: Annotated[str, Form()] = 'both', max_results: Annotated[int, Form()] = 8, min_score: Annotated[int, Form()] = 15, queries: Annotated[list[str] | None, Form()] = None):
    result = await discover_influencers(db, platform=platform, queries=queries, max_results_per_query=max_results, min_collab_score=min_score)
    msg = f"کشف: {result['queries_run']} جستجو، {result['profiles_found']} پروفایل، {result['new_saved']} جدید، {result['duplicates']} تکراری."
