...
def layout(title: str, body: str, token: str = '') -> HTMLResponse:
    ...
    return HTMLResponse(f'<!doctype html><html lang="fa"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{h(title)}</title>{css}</head><body><div class="wrap"><div class="hero"><h1>{h(title)}</h1><div class="muted">مدیریت کمپین جذب فروشنده، پیام دستی، گزارش و پیگیری</div><a class="btn btn2" href="/">بانک لیدها</a> <a class="btn btn2" href="/campaigns">کمپین‌ها</a> <a class="btn btn2" href="/crm">CRM</a> <a class="btn btn2" href="/analytics">📊 آنالیتیکس</a></div>{body}</div>{js}</body></html>')
...
def campaigns_create(db: Session = Depends(get_db), token: Annotated[str, Form()] = '', name: Annotated[str, Form()] = '', goal: Annotated[str, Form()] = '', target_type: Annotated[str, Form()] = 'lead', target_source: Annotated[str, Form()] = '', target_category: Annotated[str, Form()] = '', target_status: Annotated[str, Form()] = '', target_city: Annotated[str, Form()] = '', message_template_id: Annotated[str, Form()] = '', message_template_b_id: Annotated[str, Form()] = '', daily_batch_size: Annotated[int, Form()] = 30):
    ...
    return RedirectResponse(url=f'/campaigns/{camp.id}', status_code=303)
...
def campaigns_populate(campaign_id: int, db: Session = Depends(get_db), token: Annotated[str, Form()] = '', skip_recent: Annotated[str | None, Form()] = '1'):
    ...
    db.commit()
    return RedirectResponse(url=f'/campaigns/{c.id}?msg={quote_plus(f"{added} عضو اضافه شد، {skipped} مورد رد شد")}', status_code=303)
...
def campaign_detail(campaign_id: int, db: Session = Depends(get_db), token: str = Query(''), msg: str = ''):
    ...
    for m in all_members:
        ...
        action_buttons = ''.join(f'<form method="post" action="/campaigns/{c.id}/members/{m.id}/status" style="display:inline"><input type="hidden" name="status" value="{h(code)}"><button class="btn2">{h(label)}</button></form>' for code,label in [('sent','پیام ارسال شد'),('replied','جواب داد'),('registered','ثبت‌نام کرد'),('irrelevant','نامرتبط'),('skipped','رد شد')])
    ...
    <div class="card"><form method="post" action="/campaigns/{c.id}/populate"><label><input type="checkbox" name="skip_recent" value="1" checked> حذف کسانی که ۷ روز اخیر پیام گرفته‌اند</label><button>افزودن مخاطبین هدف به کمپین</button></form><a class="btn2" href="/campaigns/{c.id}/export.csv">خروجی CSV کمپین</a></div>
